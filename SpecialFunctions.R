...
# 28/01/2015  :  Silverman's rule for bandwidth
# 28/01/2015  :   Specialreg with marginal effects
>>>>>>> 6771a03bc293ee5140320c647efdab5ba49120cf
>>>>>>> origin/master
...
specialreg.fitMarg <- function(y,v, endo, exo,  iv,
  ...
  # Main function for special regressor (01/04/2015) 
{
  # Defining the type of vectors
  y <- as.factor(y)
  v <- as.numeric(v)  
...
#     - specialreg.fit computes the special reg coefs
#     - specialreg.mfx uses specialreg.fit  to compute several version of marginal effects


## Integrating Specialreg.fit into specialreg.mfx

# First we use the elements in specialreg.fitMarg to define specialreg.fit
...
specialreg.fit <- function(y,v, endo, exo,  iv,
  ...
  # Estimating special regressor coefs
{
  # Imposing the type of vectors
  y <- as.factor(y)
  v <- as.numeric(v)  
  
  ## ---- Should not need this below anymore ----  !!!
  #   endo <- as.matrix(endo)
  #   exo <- as.matrix(exo)
  #   iv <- as.matrix(iv)
  ...
  print(" End of specialreg.fit ")
  # HERE we need to make a list of all results interesting for mfx computation
  print(" --------xxxxx-------")
  
  special.results <- list(coefs = spe.ivreg$coefficients, values = spe.ivreg$fitted.values )
...
) 
{
  ...
  # getting the results of special reg estimation
  special.results <- specialreg.fit(y,v, endo, exo,  iv,  
                                    trimtype = trimtype,
                                    trimlevel = trimlevel, 
                                    hetero = hetero, 
                                    hetv = hetv, 
  ...
  # Imposing the type of vectors (as in specialreg.fit)
  y <- as.factor(y)
  v <- as.numeric(v)  
  
  ## Step 5 in Stata procedure : Computing marginal effects 
  # we keep close to stata notations 
  beta.hat <- as.matrix(special.results$coefs)
  xbeta.hat <- special.results$values
  length(xbeta.hat) <- length(y)    # One has to have the same length  (Trimming + estimation)
  
  # Computing the Silverman bandwidth for that np regression
